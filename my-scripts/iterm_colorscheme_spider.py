...
def main():
    """
    新式类main实现模拟浏览器访问爬取itermcolorschemes网站的所有主题的url
    写入到itermcolor.txt文件中
    通过requests请求模拟浏览器访问网站
    """
    # 注入headers
    headers = {
    ...
    bar = Bar('Downloading', max=len(theme_https_list), fill='#', suffix='%(percent)d%%')
    for i in range(len(theme_https_list)):
        # print(i)
        response = requests.get(theme_https_list[i], headers=headers)
        if response.status_code == 200:
            data = response.text
            with open(theme_name_list[i]+".itermcolors", mode="w+", encoding="UTF-8") as f:
                f.write(data)
                # print("\rDownloading %s" % theme_name_list[i], end="")
                f.close()
        else:
            print("\rConnecting %s.itermcolors falied, skipping..." % theme_name_list[i], end="")
        bar.next()
    bar.finish()
